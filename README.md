...
- **Alerts/monitoring resources are intentionally out of scope for now.** They aren't schema-groundable (metric names live in a separate monitoring API, not in `terraform providers schema -json`), and they'd require a second resource type per module. This mirrors an existing, deliberate policy in a related internal tool for the same reasons.
