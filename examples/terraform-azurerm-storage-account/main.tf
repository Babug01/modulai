...
/*
  ...
  Generated against azurerm v5.4.0 (latest at generation time). See variables.tf
  and README.md for the version-pinning policy: this floor is the exact version
  the module was generated/verified against, not an arbitrary major-version floor.
*/
#------------------------------------------------------------------------------------------------------------------------------------------
terraform {
...
/*
  ...
  - azurerm_storage_account.this: single storage account instance.
    Multiplicity (creating several accounts) is the caller's responsibility via
    `for_each` on the module block — see README.md "Creating multiple accounts".
*/
#------------------------------------------------------------------------------------------------------------------------------------------
resource "azurerm_storage_account" "this" {
  ...
  # resource — use one or the other, never both (provider docs, customer_managed_key note).
  ...
  lifecycle {
    # customer_managed_key can alternatively be managed via the standalone
    # azurerm_storage_account_customer_managed_key resource — see provider docs
    # note under `customer_managed_key`. Ignore drift here if that pattern is used instead.
    ignore_changes = []
  }
}
